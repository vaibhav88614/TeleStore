...
impl NetStream {
    ...
    async fn connect_proxy_stream(
        ...
        let socks_addr = match host {
            Host::Domain(domain) => {
                // NOTE: With hickory-resolver 0.26.x both `builder_tokio()`
                // AND `ResolverBuilder::build()` return `Result<_, NetError>`,
                // whereas in 0.25.x only `builder_tokio()` did. Use explicit
                // conversions so this works on both lines without relying on
                // an upstream `From<_> for io::Error` impl for the new error.
                ...
                let response = resolver
                    ...
                    .await
                    .map_err(|err| io::Error::new(ErrorKind::Other, err))?;
                // NOTE: With hickory-resolver 0.26.x `LookupIp` no longer
                // implements `IntoIterator`; use the explicit `iter()` method
                // which returns `LookupIpIter<'_>` yielding `IpAddr`.
